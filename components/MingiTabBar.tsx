...
//
// ТОЧКА НА «ПОМОЩИ» (Вехи 52, 54): просто точка, без числа (решение
// владельца). Горит, если по ВАЖНЫМ для меня категориям
// (users.help_notify_categories; пусто = все) есть живые посты новее
// моего последнего захода на вкладку (users.help_seen_at). Фильтр ленты
// и колокольчик на точку не влияют. Слушает help_posts (новый пост →
// зажечься) и СВОЮ строку users (открыл вкладку или сменил важные
// категории → пересчёт). Счёт — hasUnseenHelpPosts из helpService;
// при любой ошибке точки просто нет.
    ...
    (async () => {
      ...
      unsubscribe = subscribeToChanges(
        ...
        [
          ...
          // сдвинулся) или сменил важные категории — точка пересчитывается.
